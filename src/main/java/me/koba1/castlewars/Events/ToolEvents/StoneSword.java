...
public class StoneSword implements Listener {
    ...
    public void onStoneInteract(PlayerInteractEvent e) { // BANNER LOCATION
        ...
        if(e.getItem().getType() != Material.STONE_SWORD) return;
        ...
        try {
            ...
            //
                ...
                m.getConfig().set("red_banner.Location", RegionAPI.convertLocToString(e.getClickedBlock().getLocation().add(0, 1, 0)));
                m.saveConfig();
                e.getPlayer().sendMessage(MainCommand.chatcolor(Messages.get().getString("set_banner_location").replace("%team%", "Red")));
                e.setCancelled(true);
            }
