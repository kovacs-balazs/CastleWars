...
public class IronShovel implements Listener {
    ...
    public void onStoneInteract(PlayerInteractEvent e) { // BANNER LOCATION
        ...
        if(e.getItem().getType() != Material.IRON_SHOVEL) return;
...
//            }
            if (e.getAction() == Action.RIGHT_CLICK_BLOCK) {
                m.getConfig().set("lobby_teleport_location", RegionAPI.convertLocToStringYawPitch(e.getClickedBlock().getLocation().add(0, 1, 0)));
                m.saveConfig();
