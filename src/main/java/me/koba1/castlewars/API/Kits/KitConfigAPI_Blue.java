...
public class KitConfigAPI_Blue {
    ...
    public static void readBlueKit() {
        itemStacks = new LinkedHashMap<>();
        armors = new ItemStack[4];

        outer: for(String str : Kits.get().getStringList("kits.blue.items")) {
            try {
        ...
        outer: for(String str : Kits.get().getStringList("kits.blue.armor")) {
            try {
                String item = str.split(" ")[0];
                String amount = str.split(" ")[1];
