...
public class IronHoe implements Listener {
    ...
    public void onStoneInteract(PlayerInteractEvent e) { // BANNER LOCATION
        ...
        try {
            if (e.getAction() == Action.LEFT_CLICK_BLOCK) { // BLUE BANNER LOCATION
                m.getConfig().set("lobby_location_1", RegionAPI.convertLocToString(e.getClickedBlock().getLocation()));
                m.saveConfig();

                e.getPlayer().sendMessage(MainCommand.chatcolor(Messages.get().getString("lobby_1")));
                e.setCancelled(true);
            }
            //
            else if (e.getAction() == Action.RIGHT_CLICK_BLOCK) {
                m.getConfig().set("lobby_location_2", RegionAPI.convertLocToString(e.getClickedBlock().getLocation()));
                m.saveConfig();

                e.getPlayer().sendMessage(MainCommand.chatcolor(Messages.get().getString("lobby_2")));
                e.setCancelled(true);
            }
